...
client = OpenAI()
together_client = Together()  # Initialize Together client
...
anthropic_models_list = ["claude-3-opus-20240229", 
                        "claude-3-7-sonnet-latest", 
                        "claude-3-5-haiku-latest", 
                        "claude-3-sonnet-20240229", 
                        "claude-3-haiku-20240307"]

gpt_models_list = ['gpt-4o-2024-08-06', 
                    'gpt-4.5-preview-2025-02-27', 
                    'o3-mini-2025-01-31'] 

together_models_list = ["meta-llama/Llama-3.3-70B-Instruct-Turbo", 
...
def num_tokens_from_messages(messages, model="gpt-4o-mini-2024-07-18"):
    ...
    if model in {
        ...
        print("Warning: gpt-o3, gpt-4o and gpt-4o-mini may update over time. Returning num tokens assuming gpt-4o-2024-08-06.")
...
def generate_code_strict_format(before_lines, after_lines, model, program_type, 
    ...
    if model in anthropic_models_list or model in together_models_list:
        # No truncation needed for these models which have large context windows
        if model in anthropic_models_list:
    ...
    try:
        if model in gpt_models_list:
            if model == "o3-mini-2025-01-31":
                response = client.chat.completions.create(
                            ...
                            # temperature=0,
                            stop=[f"{comment_symbol} --END MISSING CODE--"],
                            reasoning_effort='high'  # Set to 'low', 'medium', or 'high' based on your needs
                        )
        ...
        elif model in together_models_list:
        ...
        if start_idx == len(start_marker) - 1 or end_idx == -1:
            ...
            # Extract the code between markers, preserving indentation
            raw_snippet = generated_code[start_idx:end_idx]

            # Remove any leading/trailing blank lines but preserve indentation
            ...
            # Join lines back together, preserving indentation
            code_snippet = '\n'.join(lines)
            
            # If code starts with blank lines, it could indicate a formatting issue
