...
def detect_natural_language(line, program_type):
    """
    Determine if a line that initially matched as code is actually natural language.
    
    Parameters:
    line (str): The line to check
    program_type (str): The programming language type ('Python' or 'Java')
    
    Returns:
    bool: True if the line is natural language, False if it's code
    """
    # Natural language typically has more words with fewer code symbols
    words = line.strip().split()
    
    # Define language-specific code symbols and keywords
    code_symbols = {
        'Python': '#:=[]{}()+-*/&|^%<>.!',
        'Java': ';{}()[]=<>+-*/&|^%!.:'
    }
    
    # Strong code indicators - always treat these as code not natural language
    code_keywords = {
        'Python': {'def', 'class', 'if', 'else', 'elif', 'for', 'while', 'try', 
                  'except', 'finally', 'with', 'import', 'from', 'as', 'return',
                  'raise', 'assert', 'pass', 'break', 'continue', 'yield', 'lambda',
                  'global', 'nonlocal', 'True', 'False', 'None'},
        'Java': {'public', 'private', 'protected', 'class', 'interface', 'enum', 
                'static', 'final', 'void', 'if', 'else', 'for', 'while', 'do',
                'switch', 'case', 'break', 'continue', 'return', 'throw', 'try',
                'catch', 'finally', 'new', 'instanceof', 'this', 'super', 'extends',
                'implements', 'package', 'import', 'true', 'false', 'null'}
    }
    
    # Check for strong code keywords that would indicate this is definitely code
    first_word = words[0].lower() if words else ""
    if first_word in code_keywords.get(program_type, set()):
        return False  # It's definitely code, not natural language
    
    # Check for quoted strings which might contain natural language within code
    has_quoted_text = ("'" in line and "'" in line[line.find("'")+1:]) or \
                      ('"' in line and '"' in line[line.find('"')+1:])
    
    # Use the appropriate symbols for the given language
    symbols = code_symbols.get(program_type, code_symbols['Java'])  # Default to Java symbols if unknown
    symbol_count = sum(line.count(symbol) for symbol in symbols)
    
    # English text patterns
    has_english_sentence_structure = False
    
    # Check for sentence structure (many words, few symbols, normal English patterns)
    if len(words) > 4 and symbol_count < 2:
        # Check first word for common natural language starters
        english_starters = {'the', 'a', 'an', 'this', 'that', 'these', 'those', 'in', 'on', 'at', 
                            'for', 'with', 'about', 'to', 'i', 'you', 'he', 'she', 'it', 'we', 'they',
                            'when', 'if', 'here', 'there', 'let', 'create', 'write', 'implement', 'use'}
        
        # If it starts with a natural language word and has few code symbols, it's likely not code
        # But don't count it as natural language if it has quoted text (could be a string in code)
        if (first_word in english_starters or (len(line) > 30 and ',' in line and ';' not in line)) and not has_quoted_text:
            has_english_sentence_structure = True
    
    # Special case: single identifiers should never be considered natural language
    if len(words) == 1 and len(words[0]) > 0:
        has_english_sentence_structure = False
    
    return has_english_sentence_structure
...
def is_code_line(line, program_type, in_block_comment=False, store_comment=True):
    ...
    # First check if line starts with comment marker - early return for comments
    ...
    if program_type == "Python":
        python_code_pattern = re.compile(
            # Python control structures with colon (CHANGED: made colon optional for incomplete code)
            r'^[\s]*(async\s+def\s+\w+.*?(:)?|def\s+\w+.*?(:)?|class\s+\w+.*?(:)?|import\s+.*|from\s+.*|'  # ← ADDED async def
            r'if\s+.*?(:)?|elif\s+.*?(:)?|else\s*:?|try\s*:?|except\s+.*?(:)?|except\s*:?|'  # ← CHANGED
            r'finally\s*:?|for\s+.*?(:)?|while\s+.*?(:)?|with\s+.*?(:)?|'  # ← CHANGED
            
            # Statements and expressions
            r'return\s+.*|return\s*|raise\s+.*|assert\s+.*|@.*|pass\s*|break\s*|continue\s*|'
            r'await\s+.*|'  # ← ADDED await expression pattern

            r'f["\'].*?["\']|'  # ← ADDED f-string pattern

            # Variable assignments and operations
            r'\w+\s*(=|\+=|-=|\*=|/=|%=|&=|\|=|\^=|<<=|>>=|\*\*=|//=).*|'
            
            # NEW: Type hints (variable annotations)
            r'\w+\s*:\s*[\w\[\],\s<>]+\s*=.*|'  # ← NEW: Type hint pattern (e.g., var: Type = value)
            
            # Function calls and indexing
            r'\w+\s*\(.*\)?|'  # ← CHANGED: Made closing parenthesis optional
            r'\w+\[.*\]?|'     # ← CHANGED: Made closing bracket optional
            r'\w+\.\w+.*|'
            
            # Method calls
            r'\w+\.\w+\s*\(.*\)?|'  # ← CHANGED: Made closing parenthesis optional
            
            # List/dict/tuple operations - UPDATED
            r'\[.*\].*|'
            r'\{.*\}.*|'
            r'\(.*\).*|'
            
            # NEW: Comprehensions (list, dictionary, generator)
            r'\[\s*.*\s+for\s+.*(\s+if\s+.*)?\]?|'  # ← NEW: List comprehension pattern
            r'\{\s*.*\s+for\s+.*(\s+if\s+.*)?\}?|'  # ← NEW: Dict comprehension pattern
            r'\(\s*.*\s+for\s+.*(\s+if\s+.*)?\)?|'  # ← NEW: Generator expression pattern
            
            # Special cases
            r'^[\s]*[\[\]\{\}]+[\s]*$|'        # Isolated brackets/braces
            r'[\w,\s]+\s*=\s*.*|'              # General variable assignments
            r'^[\s]*[\],})].*$|'               # Lines ending a list/dict/tuple
            r'.*,\s*$|'                        # Lines ending with a comma
            
            # Lambda definitions
            r'lambda\s+.*:|'
            
            # Any line with an expression (CHANGED: more specific patterns to avoid natural language false positives)
            r'\w+\s*[\+\-\*/%]\s*\w+.*|'       # ← CHANGED: Operations between identifiers
            r'\w+\s*(==|!=|<=|>=|<|>)\s*\w+.*|'# ← CHANGED: Comparisons between identifiers 
            r'\(\s*\w+.*[\+\-\*/%<>=!].*\)|'   # ← CHANGED: Operations within parentheses
        ...
        # Apply natural language detection if it initially matches as code
        if is_code:
            is_natural_language = detect_natural_language(line, "Python")
            if is_natural_language:
                is_code = False
    
    elif program_type == "Java":
        # Define regex for common Java code patterns
        java_code_pattern = re.compile(
            r'^[\s]*(import\s+(static\s+)?[\w.]+(\.\*)?\s*;|'  # Import statements with static support
            r'package\s+[\w.]+\s*;|'                     # Package declarations
            
            # Class, interface and type declarations
            r'(public|private|protected)(\s+\w+)+|'      # Access modifiers with following words
            r'(public|private|protected)\s+<.*?>\s+\w+.*\{|'  # ← ADDED: Generic methods with type bounds
            r'class\s+\w+|interface\s+\w+|@\w+|enum\s+\w+|'
            r'record\s+\w+\s*\([^)]*\)\s*\{|'            # Java records (Java 16+)
            r'(void|static|final)(\s+\w+)+|'             # Type modifiers with following words
            
            # Control structures
            r'if\s*\(|for\s*\(|while\s*\(|switch\s*\(|'  # Control flow statements
            r'try\s*\{|catch\s*\(|else\s*\{|else\s+if\s*\(|' # Exception handling and else blocks
            r'do\s*\{|'                                  # do-while loop
            r'synchronized\s*\(|'                        # Synchronized blocks
            
            # Statements
            r'return\s*.*?(;|\()?|'                      # Return statements (with optional whitespace)
            r'throw\s+.*;|'                              # Throw statements
            r'assert\s+.*;|'                             # Assert statements
            r'break\s*;|continue\s*;|'                   # Flow control statements
            
            # Declarations and assignments
            r'new\s+\w+|'                                # Object creation
            r'\w+\s*\[\s*\]\s*\w+|'                      # Array declarations
            r'\w+\s+\w+\s*=|'                            # Variable declarations with assignment
            r'\w+\s+\w+\s*;|'                            # Variable declarations without assignment
            r'[\w.]+\s*=\s*.*;|'                         # Assignment statements
            r'\w+\s*\(\s*\)\s*\{|'                       # Method definitions
            r'\w+\s*\([^)]*\)\s*\{|'                     # Method definitions with parameters
            
            # Method calls
            r'System\.out\.[^;]*;|'                      # System.out statements
            r'\w+\.\w+\([^;]*\);|'                       # Method invocations
            r'\w+\s*\([^;]*\)\s*;|'                      # Method calls with semicolon
            
            # Operators and expressions
            r'\w+\s*(\+\+|--)\s*;|'                      # Increment/decrement
            r'\w+\s*(\+=|-=|\*=|/=|%=|&=|\|=|\^=|<<=|>>=)\s*[^;]*;|' # Compound assignments
            
            # Lambda expressions
            r'\w+\s*=\s*\([^)]*\)\s*->|'                 # Lambda assignments
            r'\([^)]*\)\s*->|'                           # Lambda expressions
            
            # Syntax elements and fragments
            r'^\s*[\{\}]\s*$|'                           # Standalone braces
            r'^\s*\}|^\s*\)|^\s*\);|'                    # Lines starting with closing syntax
            r'case\s+[^:]+:|default\s*:|'                # Case statements
            
            # Generics
            r'\w+<[^>]+>|'                               # Generic type usage
            r'Map<[^>]+>|List<[^>]+>|Set<[^>]+>|Collection<[^>]+>|' # Common generic collections
            
            # Incomplete but valid Java fragments
            r'\w+\s*\(.*|'                               # Partial method calls or declarations
            r'if\s*\(.*|for\s*\(.*|while\s*\(.*|'        # Partial control statements
            r'.*&&.*|.*\|\|.*|'                          # Logical expressions
            
            # Single identifiers (last resort)
            r'\w+\.\w+|'                                 # Property access
            r'\w+\[\w+\]|'                               # Array access
            r'[A-Z][a-zA-Z0-9_]*(?:\.[A-Z][a-zA-Z0-9_]*)+|' # Class references with package
            r'^\s*\w+$'                                  # ← ADDED: Single variable identifiers
            r').*', re.DOTALL)
        
        # First check if it matches a Java code pattern
        is_code = bool(java_code_pattern.match(line))
        # Apply natural language detection if it initially matches as code
        if is_code:
            is_natural_language = detect_natural_language(line, "Java") 
            if is_natural_language:
                is_code = False
...
def clean_code(code, program_type, check_brackets=False, keep_comments=False, 
               return_first_code_snippet=True, remove_thinking_process=True, 
               thinking_tokens=[('<think>', '</think>')]):
    """
    Clean the provided code by removing non-code syntax, language dividers, comments,
    and optional thinking process text between designated tokens.
    
    Parameters:
    code (str): The code to be cleaned.
    program_type (str): The programming language of the code.
    check_brackets (bool): Whether to check for unclosed brackets.
    keep_comments (bool): Whether to keep comments in the output.
    return_first_code_snippet (bool): Whether to extract code from markdown block first.
    remove_thinking_process (bool): Whether to remove thinking process sections.
    thinking_tokens (list): List of tuples (start_token, end_token) defining thinking process sections.
    
    Returns:
    str: The cleaned code.
    """
    # Extract code from markdown block if requested
    if return_first_code_snippet:
        extracted_code = extract_markdown_code_block(code, program_type)
        if extracted_code is not None:
            return extracted_code
    
    # Remove thinking process sections if requested
    if remove_thinking_process and thinking_tokens:
        for start_token, end_token in thinking_tokens:
            # Remove sections between thinking tokens
            pattern = re.escape(start_token) + r'.*?' + re.escape(end_token)
            code = re.sub(pattern, '', code, flags=re.DOTALL)
            
            # Remove any leftover thinking tokens
            code = code.replace(start_token, '').replace(end_token, '')
    
    # Continue with existing code cleanup
    code = re.sub(r'^```.*$', '', code, flags=re.MULTILINE)  # Remove Markdown code fences
    lines = code.split('\n')
    processed_lines = []
    ...
    for line in lines:
        # Skip empty lines that might have been created from removing thinking tokens
        if not line.strip():
            continue
        ...
        # Update structure depth for brackets
        for bracket in '([{':
