...
run_analysis() {
    ...
    if [[ " ${model_list[@]} " =~ " ${gen_model} " ]]; then
        ...
        command="python model_inference.py $python_file $json_file --read_dependency_results --update_def_line --gen_model $gen_model --code_gen_mode $code_gen_mode > $stdout_path 2> $stderr_path"
    else
