...
def sanitize_model_name(model_name, systematic_name=False):
    """
    Sanitize model names for file paths to ensure compatibility.
    
    Args:
        model_name (str): The original model name
        systematic_name (bool): Whether to create file-system safe names
            - If True: Always sanitize all model names to be file-system safe
            - If False: Only apply predefined mappings for known models
        
    Returns:
        str: Sanitized model name safe for file paths
    """
    # Special mappings for known models (always applied)
    model_mapping = {
    ...
    if model_name in model_mapping:
        # Use predefined clean names for specific models
        return model_mapping[model_name]
    elif systematic_name:
        # For other models, replace problematic characters when systematic_name=True
        sanitized = model_name.replace('/', '-').replace('.', '-')
        return sanitized
    else:
        # Return the original name if not in mapping and systematic_name=False
        return model_name


def modify_output_path(source_code_path, output_file_path, gen_model, code_gen_mode, save_to_log_folder=True, systematic_name=True):
    ...
    sanitized_model_name = sanitize_model_name(gen_model, systematic_name)

    # Create a safe filename (remove remaining special characters)
    sanitized_model_name = sanitized_model_name.replace(')', '').replace('(', '').replace(' ', '_')

    # Construct the new file name with sanitized_model_name, code_gen_mode, the last part of the file name without extension, and current time
    new_file_name = f"{sanitized_model_name}_{code_gen_mode}_{file_name_without_extension}_{current_time}.csv"
